...
function spriteClass() {
	...
	this.setSprite = function(newSpriteSheet,
		...
		if (typeof newSpriteSheet != "undefined"){ spriteSheet = newSpriteSheet; }
		else {
			return;
		}
		frameX = 0;
	...
	this.draw = function(x, y, flip) {
		...
		if (drawFrame) {
			...
			if(getImgData && tintThisFrame) {
				...
				//if (toFlip){canvasContext.restore();}
				//if (toFlip){canvasContext.scale(-1.0, 1.0);}
			}
